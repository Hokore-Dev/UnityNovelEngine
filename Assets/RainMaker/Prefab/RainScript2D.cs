...
namespace DigitalRuby.RainMaker
{
    public class RainScript2D : BaseRainScript
    {
        ...
        public void RainStart(float time)
        {
            LeanTween.value(gameObject, 0, 0.5f, time).setOnUpdate(RainValueUpdate);
        }

        public void RainEnd(float time)
        {
            LeanTween.value(gameObject, 0.5f, 0, time).setOnUpdate(RainValueUpdate).setOnComplete(()=> 
            {
                gameObject.SetActive(false);
            });
        }
        
        private void RainValueUpdate(float value)
        {
            RainIntensity = value;
        }
