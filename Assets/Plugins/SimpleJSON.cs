...
//	LINK : http://wiki.unity3d.com/index.php/SimpleJSON
/* * * * *
    ...
    public class JSONClass : JSONNode, IEnumerable
    {
        ...
        public override string ToString()
        {
            ...
            foreach(KeyValuePair<string, JSONNode> N in m_Dict)
            {
                if(result.Length > 2)
                    result.Append(", ");
                result.AppendFormat("\"{0}\":{1}", Escape(N.Key), N.Value.ToString());
            }
            result.Append("}");
            return result.ToString();
        }
        public override string ToString(string aPrefix)
        {
            ...
            foreach(KeyValuePair<string, JSONNode> N in m_Dict)
            {
                if(result.Length > 3)
                    result.Append(", ");
                result.AppendFormat("\n{0}   \"{1}\" : {2}", aPrefix, Escape(N.Key), N.Value.ToString(aPrefix + "   "));
            }
            result.AppendFormat("\n{0}}", aPrefix);
            return result.ToString();
        }
